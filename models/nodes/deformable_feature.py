...
class DeformableNodes_feature(RigidNodes_feature):
    def __init__(
            self,
            **kwargs
    ):
    ...
    def deform_gaussian_points(
        ...
        cur_normalized_time = torch.tensor(cur_normalized_time, dtype=torch.float32, device=self.device).unsqueeze(
            0).repeat(means.shape[0], 1)
