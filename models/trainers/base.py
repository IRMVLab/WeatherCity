...
from utils.camera import get_points_depth_in_depth_map, get_points_from_depth
...
class BasicTrainer(nn.Module):
    def __init__(
        ...
        # dataset infos
        self.num_train_images = len(dataset.train_image_set)
        ...
        self.test_set_indices = dataset.test_timesteps # will be override
    ...
    def initialize_optimizer(self) -> None:
        ...
        for params_name, params in self.param_groups.items():
            ...
            if not hasattr(self.model_config, class_name):
                class_name = class_name.split(".")[0]
            class_cfg = self.model_config.get(class_name)
            assert class_cfg is not None, f"param group {params_name} not found in config"

            class_optim_cfg = class_cfg["optim"]

            if not hasattr(class_optim_cfg, component_name):
                component_name = component_name.split(".")[0]
            raw_optim_cfg = class_optim_cfg.get(component_name, None)
            assert raw_optim_cfg is not None, f"param group {params_name} not found in config"
    ...
    def postprocess_per_train_step(self, step: int) -> None:
        ...
        # if step % 100 == 0:
        #     logger.info(f"Step {step} - Grads statistics:")
        #     logger.info(f"  Max: {grads.max().item():.6f}")
        #     logger.info(f"  Min: {grads.min().item():.6f}")
        #     logger.info(f"  Mean: {grads.mean().item():.6f}")
        #     logger.info(f"  Std: {grads.std().item():.6f}")
    ...
    def render_gaussians(
        ...
        **kwargs,
        ...
        if update_info:
            rgb, depth, opacity, self.info = render_fn(return_info=update_info)
        else:
            rgb, depth, opacity = render_fn(return_info=update_info)
        results = {
            "rgb_gaussians": rgb,
            "depth": depth, 
            "opacity": opacity
    ...
    def compute_losses(
        ...
        for class_name in self.gaussian_classes.keys():
            class_reg_loss = self.models[class_name].compute_reg_loss(weather)
