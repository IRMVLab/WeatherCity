...
from models.gaussians import FeatureGaussians
...
class RigidNodes_feature(FeatureGaussians):
    def __init__(
            self,
            **kwargs
    ):
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int = 2) -> Tuple:
        ...
        return new_means, new_feature, new_opacities, new_scales, new_quats, new_ids
    ...
    def dup_gaussians(self, dup_mask: torch.Tensor) -> Tuple:
        ...
        return dup_means, dup_feature, dup_opacities, dup_scales, dup_quats, dup_ids
    ...
    def get_gaussians(self, cam: dataclass_camera, weather: str) -> Dict[str, torch.Tensor]:
    ...
    def get_instance_activated_gs_dict(self, ins_id: int) -> Dict[str, torch.Tensor]:
        ...
        gaussian_dict = {
            ...
            "feature": self._features[pts_mask],
    ...
    def compute_reg_loss(
        ...
        loss_dict = super().compute_reg_loss(weather)
    ...
    def remove_instances(self, remove_id_list: List[int]) -> None:
        ...
        for ins_ids in remove_id_list:
            ...
            self._features = Parameter(self._features[mask])
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_features": self._features[self.point_ids[..., 0] == id],
    ...
    def replace_instances(self, replace_dict: Dict[int, int]) -> None:
        ...
        for ins_id, new_id in replace_dict.items():
            ...
            self._features = Parameter(torch.cat([self._features, new_gaussian["_features"]], dim=0))
    ...
    def export_gaussians_to_ply(self, alpha_thresh: float, weather: str, instance_id: List[int] = None) -> Dict[str, torch.Tensor]:
        pts_mask = self.point_ids[..., 0] == instance_id

        means = self._means[pts_mask]
        direct_color = self.colors(weather)[pts_mask]
