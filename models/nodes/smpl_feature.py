...
from models.nodes import RigidNodes_feature
...
class SMPLNodes_feature(RigidNodes_feature):
    def __init__(
            self,
            **kwargs
    ):
    ...
    def compute_reg_loss(
        ...
        if knn_reg is not None:
            ...
            if not self.ctrl_cfg.freeze_feature:
                rgbs = self.mlp_color[weather](self._features)
                valid_shs_dc = rgbs.reshape(self.num_instances, self.smpl_points_num, 3)[
    ...
    def state_dict(self) -> Dict:
        state_dict = FeatureGaussians.state_dict(self)
        state_dict.update({
    ...
    def load_state_dict(self, state_dict: Dict, **kwargs) -> str:
        ...
        if self.use_voxel_deformer:
            self.template.voxel_deformer.enable_voxel_correction()
        msg = FeatureGaussians.load_state_dict(self, state_dict, **kwargs)
        self.update_knn(self._means)
        return msg
        ...
        for id in ids:
            if id not in gaussian_dict:
                instance_raw_dict = {
                    ...
                    "_features": self._features_dc[self.point_ids[..., 0] == id],
    ...
    def replace_instances(self, replace_dict: Dict[int, int]) -> None:
        ...
        for ins_id, new_id in replace_dict.items():
            ...
            self._features.data[self.point_ids[..., 0] == ins_id] = new_gaussian["_features"]
    ...
    def export_gaussians_to_ply(
            self, alpha_thresh: float, weather: str, instance_id: List[int] = None, specific_frame: int = 0,
    ) -> Dict[str, torch.Tensor]:
        ...
        color = self.colors(weather)[self.point_ids[..., 0] == ins_id]
        return {
