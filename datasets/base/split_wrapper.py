...
class SplitWrapper(torch.utils.data.Dataset):
    ...
    def get_image(self, idx, camera_downscale, weathers : List) -> Tuple[dict, dict]:
        ...
        for w in weathers:
            image_infos[w], cam_infos[w] = self.datasource.get_image(self.split_indices[idx], w)
        self.datasource.reset_downscale_factor()
        return image_infos, cam_infos

    def next(self, camera_downscale, weathers: List) -> Tuple[dict, dict]:
        ...
        image_infos, cam_infos = {}, {}
        for w in weathers:
            image_infos[w], cam_infos[w] = self.datasource.get_image(img_idx, w)
        self.datasource.reset_downscale_factor()
