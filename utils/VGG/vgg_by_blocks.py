...
class VGG_BY_BLOCKS:
    def __init__(self, vgg, img_torch, style_layers, inner_block_size=512, content_layers=[], overlap_margin_size=256,
                 verbose_mode=True):
    ...
    def global_content_loss_with_gradient(self, content_targets, content_weights):
        ...
        # Step 1: Compute content loss gradient using backprop for each block: Add content loss parts to full_loss
        full_loss = torch.tensor(0., device=self.img_torch.device)
        for block_idx in range(self.nblocks):
            # extract block
            block_torch = self.extract_block_with_ov(block_idx)
            # compute vgg layers of blocks
            vgg_out = self.vgg(block_torch, self.content_layers)
            ...
            content_layer_grad_outputs = content_weights[0] * 2 / torch.numel(content_targets[0]) * (
                    vgg_out[0] - target_content_block)
            # autograd for block:
            block_torch_grad = torch.autograd.grad(vgg_out, block_torch, [content_layer_grad_outputs])[0]
            # add inner part of this gradient to the full gradient
            inner_coord_tuple = self.inner_block_original_coord_list[block_idx]
            tuple_ov = self.block_overlap_list[block_idx]
            block_grad_crop = block_torch_grad[:, :, tuple_ov[0]:block_torch.shape[2] - tuple_ov[1],
                              tuple_ov[2]:block_torch.shape[3] - tuple_ov[3]]
            self.img_torch.grad[:, :,
            inner_coord_tuple[0]:inner_coord_tuple[1],
            inner_coord_tuple[2]:inner_coord_tuple[3]].add_(block_grad_crop / torch.numel(block_grad_crop))
            # add block contribution to content loss: mse of inner block:
            b, c, h, w = vgg_out[0].shape
            tuple_ov = self.block_overlap_list[block_idx]
            tuple_ov_layer = (x // self.content_scale_list[0] for x in tuple_ov)
            top_ov, bot_ov, left_ov, right_ov = tuple_ov_layer
            vgg_out_layer_crop = vgg_out[0][:, :, top_ov:(h - bot_ov), left_ov:(w - right_ov)]
    ...
    def global_content_loss(self, content_targets, content_weights):
        # content_targets is a list of a single torch tensor
    ...
    def global_Gram_loss_with_gradient(self, style_targets, style_weights):
        ...
        for block_idx in range(self.nblocks):
            ...
            loss_layer_grad_outputs = self.Gram_loss_layer_grad_outputs(style_weights, vgg_out[:len(self.style_layers)],
                                                                        diff_Gram_matrices)
            # autograd for block:
    ...
    def global_content_plus_Gram_loss_with_gradient(self, style_targets, style_weights,
        ...
        for block_idx in range(self.nblocks):
            ...
            target_content_block = content_targets[0][:, :,
                                   ...
                                   full_coord_tuple_layer[2]:full_coord_tuple_layer[3]]
            content_layer_grad_outputs = content_weights[0] * 2 / torch.numel(content_targets[0]) * (
                    vgg_out[len(self.style_layers)] - target_content_block)
    ...
    def global_content_plus_Gram_means_stds_loss_with_gradient(self,
        ...
        for block_idx in range(self.nblocks):
            ...
            inner_coord_tuple[0]:inner_coord_tuple[1],
            inner_coord_tuple[2]:inner_coord_tuple[3]].add_(
              block_grad_crop / torch.numel(block_grad_crop))
            # add block contribution to content loss: mse of inner block:
            b, c, h, w = vgg_out[len(self.style_layers)].shape
            tuple_ov = self.block_overlap_list[block_idx]
            tuple_ov_layer = (x // self.content_scale_list[0] for x in tuple_ov)
    ...
    def visualize_content_layer_stacked(self, save_path=None):
        """
        Visualize all channels stacked activation map (1:1 superposition) of content layer

        Usage:
            vgg_blocks = VGG_BY_BLOCKS(vgg, rgb_pred, style_layers,
                                       content_layers=['r41'], verbose_mode=False)
            vgg_blocks.visualize_content_layer_stacked(save_path='output.png')
        ...
        if len(self.content_layers) == 0:
            print("ERROR: No content_layers specified, cannot visualize")
            return

        if len(self.content_layers) != 1:
            print("WARNING: Only single content layer visualization supported, using first layer")

        content_layer = self.content_layers[0]
        layer_idx = 0
        n_channels = self.content_nchannel_list[layer_idx]
        h_feat, w_feat = self.content_layers_spatial_size_list[layer_idx]

        print(f"Visualizing layer: {content_layer}")
        print(f"Feature map size: {h_feat} x {w_feat}")
        print(f"Number of channels: {n_channels}")

        # Collect global feature map
        all_features = torch.zeros(
            (1, n_channels, h_feat, w_feat),
            device=self.img_torch.device
        )

        # Iterate through all blocks to extract features
        for block_idx in range(self.nblocks):
            block_torch = self.extract_block_with_ov(block_idx)
            ...
            # Extract inner block features (remove overlap)
            b, c, h, w = vgg_out_layer.shape
            inner_coord_tuple = self.inner_block_original_coord_list[block_idx]
            inner_coord_tuple_layer = tuple(
                (x // self.content_scale_list[0] for x in inner_coord_tuple)
            )
            tuple_ov = self.block_overlap_list[block_idx]
            tuple_ov_layer = tuple(
                (x // self.content_scale_list[0] for x in tuple_ov)
            )
            top_ov, bot_ov, left_ov, right_ov = tuple_ov_layer
            ...
            # Fill into global feature map
            all_features[:, :,
            inner_coord_tuple_layer[0]:inner_coord_tuple_layer[1],
            inner_coord_tuple_layer[2]:inner_coord_tuple_layer[3]] = vgg_out_layer_crop.clone()

        # Stack all channels (1:1 superposition)
        stacked_activation = all_features.sum(dim=1)[0]  # (H, W)

        # Normalize to [0, 1]
        stacked_min = stacked_activation.min()
        stacked_max = stacked_activation.max()
        if stacked_max - stacked_min > 1e-5:
            stacked_norm = (stacked_activation - stacked_min) / (stacked_max - stacked_min)
        else:
            stacked_norm = stacked_activation * 0

        # Create figure
        fig, ax = plt.subplots(figsize=(10, 8))

        stacked_np = stacked_norm.cpu().numpy()
        im = ax.imshow(stacked_np, cmap='viridis')
        ax.set_title(f'{content_layer} - Stacked Channel Activation Map', fontsize=14, fontweight='bold')
        ax.axis('off')
        cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label('Cumulative Activation (Normalized)', fontsize=12)

        # Add statistics
        stats_text = (
            f'Channels: {n_channels}\n'
            f'Spatial size: {h_feat}×{w_feat}\n'
            f'Max value: {stacked_activation.max():.2f}\n'
            f'Min value: {stacked_activation.min():.2f}\n'
            f'Mean value: {stacked_activation.mean():.2f}'
        )
        ...
        if save_path:
            ...
            print(f"Stacked activation map saved to: {save_path}\n")
