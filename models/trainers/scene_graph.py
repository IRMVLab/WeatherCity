...
class MultiTrainer(BasicTrainer):
    ...
    def forward(
        ...
        #     #     print("k",k,v.shape,v.max(),v.min())
        #
        #     # outputs["rgb"] = outputs["rgb"]*0.5 + 0.5
        #     fog_strength = 0.02 #0.1 #0.05
        #     # depth_normalized = outputs["depth"]/50.
        #     depth = outputs["depth"].clone().detach()
        #     # print(image_infos["sky_masks"].shape,image_infos["sky_masks"].dtype,image_infos["sky_masks"].max(),image_infos["sky_masks"].min())
        #     depth[image_infos["sky_masks"].to(torch.int32)] = outputs["depth"].max()
        #     fog_strength_map = torch.exp(-depth*fog_strength)
        #     fog_strength_map = torch.clamp(fog_strength_map, min=0., max=1.)
        #     fog_color = torch.tensor([0.9, 0.9, 0.9]).to(self.device)
        #     fog_layer = torch.ones_like(outputs["rgb"], device=self.device) *fog_color
        #     outputs["rgb"] = outputs["rgb"] * fog_strength_map + fog_layer*(1-fog_strength_map)
