...
def do_evaluation(
    ...
    if dataset.test_image_set is not None and cfg.render.render_test:
        ...
        if log_metrics:
            ...
            for k, v in render_results.items():
                if k in [
                    ...
                    eval_dict[f"image_metrics/{weather}/test/{k}"] = v
            if args.enable_wandb:
                wandb.log(eval_dict)
            test_metrics_file = f"{cfg.log_dir}/metrics{post_fix}/{weather}/images_test_{current_time}.json"
            with open(test_metrics_file, "w") as f:
                json.dump(eval_dict, f)
            logger.info(f"Image evaluation metrics saved to {test_metrics_file}")

        if args.render_video_postfix is None:
            video_output_pth = f"{cfg.log_dir}/videos{post_fix}/{weather}/test_set_{step}.mp4"
        else:
            video_output_pth = (
                f"{cfg.log_dir}/videos{post_fix}/{weather}/test_set_{step}_{args.render_video_postfix}.mp4"
            )
        ...
        if args.enable_wandb:
            for k, v in vis_frame_dict.items():
                wandb.log({f"image_rendering/{weather}/test/" + k: wandb.Image(v)})
    ...
    if cfg.render.render_full:
        ...
        if log_metrics:
            ...
            for k, v in render_results.items():
                if k in [
                    ...
                    eval_dict[f"image_metrics/{weather}/full/{k}"] = v
            if args.enable_wandb:
                wandb.log(eval_dict)
            full_metrics_file = f"{cfg.log_dir}/metrics{post_fix}/{weather}/images_full_{current_time}.json"
            with open(full_metrics_file, "w") as f:
                json.dump(eval_dict, f)
            logger.info(f"Image evaluation metrics saved to {full_metrics_file}")

        if args.render_video_postfix is None:
            video_output_pth = f"{cfg.log_dir}/videos{post_fix}/{weather}/full_set_{step}.mp4"
        else:
            video_output_pth = (
                f"{cfg.log_dir}/videos{post_fix}/{weather}/full_set_{step}_{args.render_video_postfix}.mp4"
            )
        print(f"render results keys: {render_results.keys()}")
        ...
        if args.enable_wandb:
            for k, v in vis_frame_dict.items():
                wandb.log({f"image_rendering/{weather}/full/" + k: wandb.Image(v)})
        del render_results, vis_frame_dict
        torch.cuda.empty_cache()
    ...
    if render_novel_cfg is not None:
        ...
        video_output_dir = f"{cfg.log_dir}/videos{post_fix}/{weather}/novel_{step}"
...
def main(args):
    ...
    eval_weathers = ["raw"] + args.eval_weathers
    for folder in ["videos_eval", "metrics_eval"]:
        for w in eval_weathers:
            os.makedirs(os.path.join(log_dir, folder, w), exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_weathers", help="weather type", nargs='*', type=str, required=True)
