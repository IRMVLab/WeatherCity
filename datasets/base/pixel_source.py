...
class CameraData(object):
    def __init__(
        ...
        self.device = device
        self.load_weather = ["raw"] + load_weather

        assert "rain" in self.load_weather, self.load_weather
    ...
    def create_all_filelist(self):
        ...
        """
        # ---- define filepaths ---- #
        img_filepaths = {k: [] for k in self.load_weather}
        ...
        logger.info(f"start loading images from {self.start_timestep} to {self.end_timestep}")
        for t in range(self.start_timestep, self.end_timestep):
            for weather in self.load_weather:
                img_filepaths[weather].append(
                    os.path.join(self.data_path, "images", weather, f"{t:03d}_{self.cam_id}.jpg")
                )
        ...
        self.img_filepaths = {}
        for weather in self.load_weather:
            self.img_filepaths[weather] = np.array(img_filepaths[weather])
    ...
    def load_images(self):
        ...
        for weather in self.load_weather:
            ...
            for ix, fname in tqdm(
                enumerate(self.img_filepaths[weather]),
                desc=f"Loading {weather} images",
                dynamic_ncols=True,
                total=len(self.img_filepaths[weather]),
            ):
            ...
            self.images[weather] = torch.from_numpy(np.stack(images, axis=0)) / 255
    ...
    def to(self, device: torch.device):
        ...
        for keys in self.images.keys():
            self.images[keys] = self.images[keys].to(device)
    ...
    def get_image(self, frame_idx: int, weather: str = "raw") -> Dict[str, Tensor]:
        """
        ...
        assert weather in self.load_weather, f"Weather {weather} not in {self.load_weather}"

        if self.images[weather] is not None:
            rgb = self.images[weather][frame_idx]
...
class ScenePixelSource(abc.ABC):
    ...
    def get_nearest_idx(self, img_idx: int) -> int:
        """Sample a temporally nearby frame index for the SAME camera.

        The sampling strategy follows a mixed schedule:
        - With probability 0.8: sample a short-range offset from {±1..±5}
        - With probability 0.2: sample a mid-range offset from {±6..±10}

        Offsets that would cross sequence boundaries are excluded.

        Args:
            img_idx: Global image index in [0, num_frames * num_cams).

        Returns:
            Global image index of the sampled nearest frame (same camera).
        """
        # allow passing a scalar tensor
        ...
        def _valid_deltas(min_d: int, max_d: int):
            deltas = []
            for d in range(min_d, max_d + 1):
                if frame_idx - d >= 0:
                    deltas.append(-d)
                if frame_idx + d < self.num_frames:
                    deltas.append(d)
            return deltas

        short_deltas = _valid_deltas(1, 5)
        mid_deltas = _valid_deltas(6, 10)

        # choose short vs mid; fallback if one side has no valid candidates
        use_short = bool((torch.rand(()) < 0.8).item())
        candidates = short_deltas if use_short else mid_deltas
        if len(candidates) == 0:
            candidates = mid_deltas if use_short else short_deltas
        if len(candidates) == 0:
            return img_idx

        delta = candidates[int(torch.randint(0, len(candidates), (1,)).item())]
        nearest_frame_idx = frame_idx + delta
        return nearest_frame_idx * self.num_cams + unique_cam_idx

    def get_image(self, img_idx: int, weather: str = "raw") -> Dict[str, Tensor]:
        """
        ...
        for cam_id in self.camera_list:
            if unique_cam_idx == self.camera_data[cam_id].unique_cam_idx:
                return self.camera_data[cam_id].get_image(frame_idx, weather)
