...
def render_images(
    ...
    if compute_metrics:
        ...
        logger.info(f"Eval over {num_samples} images for weather {weather}:")
        ...
        # logger.info(f"\tPred Content Loss: {render_results['pred_content_loss']:.4f}")
        # logger.info(f"\tOrig Content Loss: {render_results['orig_content_loss']:.4f}")
...
def render(
    ...
    Parameters:
        ...
        weather: Weather condition for rendering.
    ...
    with torch.no_grad():
        ...
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            ...
            image_infos_dict, cam_infos_dict = dataset.get_image(i, camera_downscale, ["raw", weather])
            image_infos, cam_infos = image_infos_dict[weather], cam_infos_dict[weather]
            ...
            # render the image
            results = trainer(image_infos, cam_infos, weather)
            ...
            if compute_metrics:
                ...
                # pred_content_loss = trainer.compute_content_loss(
                #     outputs=results,
                ...
                # )["content_loss"]
                # orig_content_loss = trainer.compute_content_loss(
                ...
                #     content_cam_infos=raw_cam_infos,
                # )["content_loss"]
                # logger.info(f"Frame {i}: PSNR {psnr:.4f}, SSIM {ssim_score:.4f}, Content Loss {pred_content_loss.item():.4f}")
                psnrs.append(psnr)
                ssim_scores.append(ssim_score)
                lpipss.append(lpips.item())
                # pred_content_losses.append(pred_content_loss.item())
                # orig_content_losses.append(orig_content_loss.item())
    ...
    # results_dict["pred_content_loss"] = non_zero_mean(pred_content_losses) if compute_metrics else -1
    # results_dict["orig_content_loss"] = non_zero_mean(orig_content_losses) if compute_metrics else -1
...
def render_novel_views(trainer, render_data: list, save_path: str, fps: int = 30, weather: str = 'raw') -> None:
    """
...
def save_concatenated_videos(
    ...
    for i in trange(num_timestamps, desc="saving video", dynamic_ncols=True):
        ...
        for key in keys:
            ...
            source_key = (
                key.replace("mask", "opacities")
                if "mask" in key and key != "gt_sky_masks"
                else key
            )
            if source_key not in render_results or len(render_results[source_key]) == 0:
                continue
            frames = render_results[source_key][i * num_cams : (i + 1) * num_cams]
